...
export default function FriendsPanel() {
  ...
  async function onAdd(e: FormEvent) {
    ...
    try {
      ...
      setAddSuccess(json.autoAccepted ? "You're now friends!" : "Request sent");
      setUsername("");
      await load();
    } catch {
  ...
  return (
    <div className="flex flex-col gap-6">
      <h1 className="font-display text-3xl text-bone md:text-4xl">FRIENDS</h1>

      {/* Add a friend */}
      <section className="rounded-2xl border border-coal-600 bg-coal-800 p-6">
        <h2 className="font-display text-2xl text-bone">Add a friend</h2>
      ...
      {loading && <p className="font-mono text-sm text-bone/40">Loading…</p>}
      {loadError && (
        <div className="rounded-2xl border border-coal-600 bg-coal-800 p-6">
          <p className="text-sm text-coral">Couldn&apos;t load friends.</p>
          <button
            onClick={load}
      ...
      {data && (
        <>
          ...
          {data.incoming.length > 0 && (
            <section className="rounded-2xl border border-coal-600 bg-coal-800 p-6">
              <h2 className="font-display text-2xl text-bone">Requests</h2>
          ...
          {data.outgoing.length > 0 && (
            <section className="rounded-2xl border border-coal-600 bg-coal-800 p-6">
              <h2 className="font-display text-2xl text-bone">Sent</h2>
              <ul className="mt-4 flex flex-col divide-y divide-coal-600">
                {data.outgoing.map((p) => (
                  <li key={p.friendshipId} className="flex items-center justify-between gap-3 py-3">
                    <div>
          ...
          {/* Friends */}
          <section className="rounded-2xl border border-coal-600 bg-coal-800 p-6">
            <h2 className="font-display text-2xl text-bone">Your friends</h2>
            {data.friends.length === 0 ? (
              <p className="mt-3 text-sm text-bone/50">
                No friends yet — add one above to start a leaderboard. They&apos;ll need a FitMeter
                account with the username you enter.
              </p>
            ) : (
              <ul className="mt-4 flex flex-col divide-y divide-coal-600">
                {data.friends.map((p) => (
